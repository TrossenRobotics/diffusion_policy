...
class TrossenCleaningImageDataset(BaseImageDataset):
    """
    Dataset for the trash-in-bin cleaning task.

    Actions are 7D: [x, y, z, rx, ry, rz, gripper_width]
    Observations include camera images + robot_eef_pose (6D) + robot_gripper_width (1D).
    """
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            if 'pose' in key:
                # EEF pose is always 6D; 2D is the sim-only push-T shortcut
                assert tuple(shape) in [(2,), (6,)], \
                    f"Unexpected pose shape {shape} for key '{key}'"

    action_shape = tuple(shape_meta['action']['shape'])
    # 7D = 6D pose + 1D gripper width (cleaning task)
    # 6D = pose only (push-cube)
    # 2D = sim push-T
    assert action_shape in [(2,), (6,), (7,)], \
        f"Unexpected action shape {action_shape}"
    ...
    # trim action dimensions if needed (only for 2D sim push-T case)
    if action_shape == (2,):
